...
if __name__ == '__main__':
    ...
    if args.init_checkpoint is not None:
        ...
        if args.model_card == 'gpt2.md':
            lm_net.load_weight(torch.load(args.init_checkpoint, map_location=torch.device('cpu')))
            # "/leonardo_work/IscrC_EPeRLLM/NLG/pretrained_checkpoints/gpt2-medium-pytorch_model.bin"
        elif args.model_card == 'gpt2.lg':
            lm_net.load_weight(torch.load("/leonardo_work/IscrC_EPeRLLM/NLG/pretrained_checkpoints/gpt2-large-pytorch_model.bin", map_location=torch.device('cpu')))

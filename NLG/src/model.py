...
def symmetric_KL_loss(p, q, attention_mask, reduction='batchmean'):
    ...
    loss = F.kl_div(F.log_softmax(p, dim=-1, dtype=torch.float32),
            ...
            F.kl_div(F.log_softmax(q, dim=-1, dtype=torch.float32),
                     F.softmax(p.detach(), dim=-1, dtype=torch.float32), reduction=reduction)         # Symmetric KL

    #loss = F.kl_div(F.log_softmax(p, dim=-1, dtype=torch.float32),
    #                0.5*(F.softmax(q.detach(), dim=-1, dtype=torch.float32)+F.log_softmax(p, dim=-1, dtype=torch.float32)), reduction=reduction) + \
    #       F.kl_div(F.log_softmax(q, dim=-1, dtype=torch.float32),
    #                0.5*(F.softmax(p.detach(), dim=-1, dtype=torch.float32)+F.log_softmax(p, dim=-1, dtype=torch.float32)), reduction=reduction)           # Jensen–Shannon divergence
